...
parser.add_argument("-r", "--chr_size", dest="chr_size", help="path to file containing the chromosome size of the genomic reference")
...
chr_size_df = pd.read_csv(chr_size, sep="\t",header=None)
...
with open(vcf, 'r', errors="replace") as f1:
    with open(vcf_converted, 'w') as f2:
        ...
        conv_pos_df.POS = conv_pos_df.POS.astype(int)
        conv_pos_order_df = conv_pos_df.sort_values(by=['#CHROM','POS'], ascending = (True, True))
        conv_pos_order_df.POS = conv_pos_order_df.POS.astype(str)
