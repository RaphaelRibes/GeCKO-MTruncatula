...
- *GENOMIC_REFERENCE_CHR_SIZE:*&nbsp;&nbsp;&nbsp;If the input bams (from the BAMS_LIST) result from an extraction of reads mapping to specific genomic zones (i.e. CREATE_SUB_BAMS was set to TRUE during the mapping step) and you want this workflow's output vcf file to give the variants positions in the whole genomic reference, then please provide here the path to the reference_chr_size.txt file containing your genomic reference chromosomes sizes. This file is automatically created by the READS_MAPPING workflow when CREATE_SUB_BAMS is set to TRUE, and stored in WORKFLOWS_OUTPUTS/READS_MAPPING. Otherwise leave blank ("").  
