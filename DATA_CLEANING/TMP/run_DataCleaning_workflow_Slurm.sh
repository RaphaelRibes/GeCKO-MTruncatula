...
## DRYRUN ##
#snakemake --snakefile ${PRGR}/DataCleaning.smk -n --dag --forceall --cluster-config ${CONFIG}/cluster_config_Slurm.json --configfile ${CONFIG}/config_Slurm.yml --printshellcmds

## GENERATE PIPELINE DIAGRAM -> à lancer hors sbatch, dot est indisponible sur les noeuds de calcul ##
#snakemake --snakefile ${PRGR}/DataCleaning.smk -n --dag --forceall --cluster-config ${CONFIG}/cluster_config_Slurm.json --configfile ${CONFIG}/mini_tmp/config_mini_Slurm.yml | dot -Tsvg > data_cleaning_pipeline_mini.svg
