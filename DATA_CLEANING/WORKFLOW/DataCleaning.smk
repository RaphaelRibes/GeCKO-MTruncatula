...
rule Demultiplex_RawFastqs:
    ...
    shell:
        ...
        #"mkdir -p {demult_reports_infos_dir};"
...
rule Trimming_DemultFastqs:
    ...
    conda:
        "ENVS/conda_tools.yml"
    shell:
        "{scripts_dir}/trimming_with_cutadapt.sh --sample {wildcards.base} --trimdir {demult_trim_dir} "
...
rule Fastqc_TrimmedFastqs:
    ...
    output:
        demult_trim_reports_fastqc_dir+"/{base}_trimmed.{R}_fastqc.zip"
    conda:
        "ENVS/conda_tools.yml"
    shell:
...
rule MultiQC_TrimmedFastqs:
    input:
        expand("{demult_trim_reports_infos_dir}/tmp_trimming_cutadapt_{sample}.info", sample=samples, demult_trim_reports_infos_dir=demult_trim_reports_infos_dir),
        expand("{demult_trim_reports_fastqc_dir}/{sample}_trimmed.R1_fastqc.zip", sample=samples, demult_trim_reports_fastqc_dir=demult_trim_reports_fastqc_dir),
        expand("{demult_trim_reports_fastqc_dir}/{sample}_trimmed.R2_fastqc.zip", sample=samples, demult_trim_reports_fastqc_dir=demult_trim_reports_fastqc_dir)
...
rule Fastqc_ConcatTrimmedFastqs:
    ...
    output:
        demult_trim_reports_fastqc_dir+"/All_Samples_Concat_trimmed.{R}_fastqc.zip"
    conda:
        "ENVS/conda_tools.yml"
    shell:
        "fastqc -o {demult_trim_reports_fastqc_dir} {input} ;"
        "mv {demult_trim_reports_fastqc_dir}/{fastq_raw_base}_trimmed.{wildcards.R}_fastqc.html {demult_trim_reports_fastqc_dir}/All_Samples_Concat_trimmed.{wildcards.R}_fastqc.html ;"
        "mv {demult_trim_reports_fastqc_dir}/{fastq_raw_base}_trimmed.{wildcards.R}_fastqc.zip {demult_trim_reports_fastqc_dir}/All_Samples_Concat_trimmed.{wildcards.R}_fastqc.zip"
...
rule MultiQC_Global:
    input:
        ...
        demult_trim_reports_fastqc_dir+"/All_Samples_Concat_trimmed.R1_fastqc.zip",
        demult_trim_reports_fastqc_dir+"/All_Samples_Concat_trimmed.R2_fastqc.zip"],
